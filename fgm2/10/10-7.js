var fgm = {
  on: function(element,sEventType,handler) {
   return element.addEventListener?element.addEventListener(sEventType,handler,false):element.attachEvent("on"+sEventType,handler)
  },
...
LazyLoad.prototype = {
  load: function() {
    ...
    var iChientHeight = document.documentElement.clientHeight + iScrollTop;
    var aParent = [];
    var oParent = null;
    var iTop = 0;
    var iBottom = 0;
    ...
    if(this.loaded(1).length != this.aImg.length){
      for (var i = 0; i < aNotLoaded.length; i++) {
        ...
        iBottom = iTop + oParent.offseiHeight;
        if((iTop > iScrollTop && iTop <iChientHeight) || (iBottom > iScrollTop && iBottom < iChientHeight) ){
